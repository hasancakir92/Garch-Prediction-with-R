...
CalculateGarch<-function(con,symbol,yahoo_symbol,startDate,endDate){
  ffrom<-as.Date(startDate) - as.difftime(730,unit="days")
  #print(ffrom)
  ...
  for (d in 0:foreLength) {
    ...
    sgarch11_spec <- ugarchspec(variance.model = list(model="sGARCH",garchOrder = c(1, 1)),mean.model = list(armaOrder = c(unname(order['p']), unname(order['q'])),include.mean=T),distribution.model="sged")
    ...
    if(nrow(isExist)>0)
    {
      ...
      update_query<-sprintf("UPDATE dbo.Symbol SET GarchLatestUpdate='%s' Where SymbolCode='%s'",day,symbol)
...
for(row in 1:nrow(res)){
  ...
  garch_latest_update <- res[row, "GarchLatestUpdate"]

  if(!is.na(garch_latest_update) && garch_latest_update==Sys.Date()-as.difftime(1,unit="days")){
    next
  }
  
  startDate="2018-12-31"
  if(!is.na(garch_latest_update)){
    startDate=garch_latest_update
  }
